(function() {
  ...
  module.exports = Token = (function() {
    ...
    Token.prototype.hasLeadingWhitespace = false;

    Token.prototype.hasTrailingWhitespace = false;
    ...
    Token.prototype.breakOutAtomicTokens = function(tabLength, breakOutLeadingSoftTabs) {
      ...
      if (this.hasSurrogatePair) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          ...
          if (token.isAtomic) {
            ...
            outputTokens.push.apply(outputTokens, token.breakOutAtomicTokens(tabLength, breakOutLeadingSoftTabs));
          }
        ...
        while (match = regex.exec(this.value)) {
          ...
          if (softTab && breakOutLeadingSoftTabs) {
            outputTokens.push(this.buildSoftTabToken(tabLength));
          } else if (hardTab) {
            breakOutLeadingSoftTabs = false;
            outputTokens.push(this.buildHardTabToken(tabLength));
          } else {
            breakOutLeadingSoftTabs = false;
            value = match[0];
    ...
    Token.prototype.getValueAsHtml = function(_arg) {
      var classes, endIndex, hasIndentGuide, html, invisibles, leadingHtml, match, startIndex, tokenIsOnlyWhitespace, trailingHtml, value;
      invisibles = _arg.invisibles, hasIndentGuide = _arg.hasIndentGuide;
      if (invisibles == null) {
        invisibles = {};
      }
      if (this.isHardTab) {
        ...
        leadingHtml = '';
        trailingHtml = '';
        if (this.hasLeadingWhitespace && (match = LeadingWhitespaceRegex.exec(this.value))) {
        ...
        if (this.hasTrailingWhitespace && (match = TrailingWhitespaceRegex.exec(this.value))) {
          tokenIsOnlyWhitespace = match[0].length === this.value.length;
