  ...
  module.exports = WindowEventHandler = (function() {
    ...
    function WindowEventHandler() {
      ...
      this.subscribe($(window), 'window:open-path', function(event, _arg) {
        var initialColumn, initialLine, pathToOpen, _ref;
        pathToOpen = _arg.pathToOpen, initialLine = _arg.initialLine, initialColumn = _arg.initialColumn;
        if (!fs.isDirectorySync(pathToOpen)) {
          return (_ref = atom.workspace) != null ? _ref.open(pathToOpen, {
      ...
      this.subscribe($(window), 'blur unload', function() {
