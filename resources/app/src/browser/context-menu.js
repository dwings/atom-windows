  ...
  module.exports = ContextMenu = (function() {
    ...
    ContextMenu.prototype.createClickHandlers = function(template) {
      ...
      for (_i = 0, _len = template.length; _i < _len; _i++) {
        item = template[_i];
        if (item.command) {
          if (item.commandOptions == null) {
            item.commandOptions = {};
          }
          item.commandOptions.contextCommand = true;
          item.commandOptions.atomWindow = this.atomWindow;
          (function(_this) {
            return (function(item) {
              return item.click = function() {
                return global.atomApplication.sendCommandToWindow(item.command, _this.atomWindow, item.commandOptions);
              };
            });
          })(this)(item);
