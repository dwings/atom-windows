(function() {
  var $, $$, SettingEditorView, SettingsPanel, View, appendArray, appendCheckbox, appendEditor, appendObject, appendSetting, getSettingTitle, isEditableArray, _, _ref,
      ...
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        ...
        _results.push((function(_this) {
          return function(editorView) {
            var defaultValue, name, type;
            name = editorView.attr('id');
            type = editorView.attr('type');
            if (defaultValue = _this.valueToString(atom.config.getDefault(name))) {
              editorView.setPlaceholderText("Default: " + defaultValue);
            }
            _this.subscribe(atom.config.observe(name, function(value) {
              var stringValue, _ref2;
              if (atom.config.isDefault(name)) {
                stringValue = '';
              } else {
                stringValue = (_ref2 = _this.valueToString(value)) != null ? _ref2 : '';
              }
              if (stringValue === editorView.getText()) {
                return;
              }
              if (value === _this.parseValue(type, editorView.getText())) {
                return;
              }
    ...
    return this.div({
      ...
      return function() {
        return _this.div({
          ...
          return _this.subview(keyPath.replace(/\./g, ''), new SettingEditorView({
            attributes: {
              id: keyPath,
              type: type
            }
